...
class SalesOrder(Base):
    ...
    customer_name: Mapped[str] = mapped_column(String(160))
    notes: Mapped[str] = mapped_column(Text, default="")
    status: Mapped[SalesOrderStatus] = mapped_column(Enum(SalesOrderStatus), default=SalesOrderStatus.open)
    quote_id: Mapped[str | None] = mapped_column(ForeignKey("quotations.id"), nullable=True, index=True)
    quote_no: Mapped[str] = mapped_column(String(32), default="")
    taxable_total: Mapped[float] = mapped_column(Float, default=0)
    cgst: Mapped[float] = mapped_column(Float, default=0)
    sgst: Mapped[float] = mapped_column(Float, default=0)
    igst: Mapped[float] = mapped_column(Float, default=0)
...
class SalesOrderLine(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    sales_order_id: Mapped[str] = mapped_column(ForeignKey("sales_orders.id", ondelete="CASCADE"))
    product_id: Mapped[str] = mapped_column(ForeignKey("products.id"))
    product_name: Mapped[str] = mapped_column(String(160))
    hsn: Mapped[str] = mapped_column(String(16), default="")
    unit: Mapped[str] = mapped_column(String(16), default="PCS")
...
class DeliveryNote(Base):
    ...
    sales_order_id: Mapped[str | None] = mapped_column(ForeignKey("sales_orders.id"), nullable=True)
    customer_id: Mapped[str] = mapped_column(ForeignKey("parties.id"))
    customer_name: Mapped[str] = mapped_column(String(160))
...
class DeliveryLine(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    delivery_id: Mapped[str] = mapped_column(ForeignKey("delivery_notes.id", ondelete="CASCADE"))
    product_id: Mapped[str] = mapped_column(ForeignKey("products.id"))
    product_name: Mapped[str] = mapped_column(String(160))
    quantity: Mapped[float] = mapped_column(Float)
...
class Dispatch(Base):
    ...
    __table_args__ = (UniqueConstraint("delivery_id", name="uq_dispatch_delivery"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=_uuid)
    dispatch_no: Mapped[str] = mapped_column(String(32), unique=True, index=True)
    dispatch_date: Mapped[date] = mapped_column(Date, index=True)
    delivery_id: Mapped[str | None] = mapped_column(ForeignKey("delivery_notes.id"), nullable=True)
    delivery_no: Mapped[str] = mapped_column(String(32), default="")
    customer_id: Mapped[str] = mapped_column(ForeignKey("parties.id"))
    customer_name: Mapped[str] = mapped_column(String(160))
    transporter: Mapped[str] = mapped_column(String(160), default="")
    vehicle_no: Mapped[str] = mapped_column(String(32), default="")
    driver_name: Mapped[str] = mapped_column(String(120), default="")
    driver_phone: Mapped[str] = mapped_column(String(32), default="")
    lr_number: Mapped[str] = mapped_column(String(64), default="")
    status: Mapped[DispatchStatus] = mapped_column(Enum(DispatchStatus), default=DispatchStatus.planned)
    delivered_on: Mapped[date | None] = mapped_column(Date, nullable=True)
    pod_ref: Mapped[str] = mapped_column(String(160), default="")
    created_by: Mapped[str] = mapped_column(String(64), default="")
...
class GRNLine(Base):
    ...
    material_id: Mapped[str] = mapped_column(ForeignKey("raw_materials.id"))
    material_name: Mapped[str] = mapped_column(String(160), default="")
    quantity: Mapped[float] = mapped_column(Float, default=0)
